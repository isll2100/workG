...
let userID = 'e06bd4b5-7ed3-43ce-be9f-9455cef9a5fa';

const proxyIPs = ['129.159.241.172'];

// if you want to use ipv6 or single proxyIP, please add comment at this line and remove comment at the next line
let proxyIP = proxyIPs[Math.floor(Math.random() * proxyIPs.length)];
// use single proxyIP instead of random
// let proxyIP = 'www.kproxy.com';
// ipv6 proxyIP example remove comment to use
// let proxyIP = "[2a01:4f8:c2c:123f:64:5:6810:c55a]"

let dohURL = 'https://129.159.241.172'; // https://cloudflare-dns.com/dns-query or https://dns.google/dns-query
		...
		try {
			...
			if (!upgradeHeader || upgradeHeader !== 'websocket') {
				...
				switch (url.pathname.toLowerCase()) {
					case '/':
						const envKey = env.URL200 ? 'URL302' : (env.URL ? 'URL' : null);
						if (envKey) {
							const URLs = await ADD(env[envKey]);
							const URL = URLs[Math.floor(Math.random() * URLs.length)];
							return envKey === 'URL200' ? Response.redirect(URL, 200) : fetch(new Request(URL, request));
						}
...
const cn_hostnames = [
	'129.159.241.172',                // Weibo - A popular social media platform
	'129.159.241.172',            // Baidu - The largest search engine in China
	'129.159.241.172',
